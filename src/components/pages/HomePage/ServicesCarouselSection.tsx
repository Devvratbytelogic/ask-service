...
function ServiceCarouselCard({
    ...
    return (
        <button
            ...
            onClick={onClick}
            className="group flex shrink-0 cursor-pointer items-center gap-2.5 rounded-xl border-[1.5px] border-slate-100 bg-slate-50 px-[18px] py-[11px] whitespace-nowrap text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:border-primaryColor/25 hover:bg-blue-light hover:text-primaryColor hover:shadow-[0_4px_12px_rgba(27,79,255,0.1)]"
        >
            <span className="text-xl">{item.emoji}</span>
            <span className="text-left">
                <span className="block text-sm font-semibold">{item.name}</span>
                <span className="block text-[11px] text-slate-400 group-hover:text-primaryColor/70">
                    {item.count}
                </span>
            </span>
        </button>
...
export default function ServicesCarouselSection() {
    ...
    return (
        <section className="overflow-hidden border-y border-slate-100 bg-white py-8">
            <p className="mb-[18px] text-center text-[11px] font-bold tracking-[1.5px] text-slate-300 uppercase">
            ...
            <div className={shouldScroll ? "relative overflow-hidden" : undefined}>
                {shouldScroll && (
                    <>
                        <div
                            aria-hidden
                            className="pointer-events-none absolute inset-y-0 left-0 z-2 w-20 bg-linear-to-r from-white to-transparent"
                        />
                        <div
                            aria-hidden
                            className="pointer-events-none absolute inset-y-0 right-0 z-2 w-20 bg-linear-to-l from-white to-transparent"
                        />
                    </>
                )}
